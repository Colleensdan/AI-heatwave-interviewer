...
# Includes English terms so the filter works with en_core_web_*.
...
NAME_FALLBACK_PATTERN = re.compile(
    ...
    r"|i\s+am\s+called|i'?m\s+called|i\s+am|i'?m)\s+"
...
def _get_spacy_stopwords() -> Set[str]:
    """Return English stop-words from spaCy (lazy import)."""
    from spacy.lang.en.stop_words import STOP_WORDS  # type: ignore[import]
